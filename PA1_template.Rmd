...
#### run PA1_driver.R (a wrapper), which starts knit2html, then sets up and loads required libraries used by the script
...
Before the plot of the 'Total Steps per Day' is shown, I will build a dataset of the steps per day, then calculate the mean and median of the steps per day

#### firstly, find the total steps per day - note, there may be missing values

There are 288 intervals of summed, recorded, steps per day, and 61 days for the experiment.
...
```
The 'activity' data yielded a mean and median of the total steps per day of **`r mStepsPerDay[1,1]`** and **`r mStepsPerDay[1,2]`**, respectively.
...
```
#### 1. make a time series plot of 5-minute interval vs average steps over the experiment
...
#head(stepsPerInterval)
```
ggplot requires that time-date be in a POSIX format
...
```
The interval with the maximum step counts is **`r maxstepsPerInterval`**

### Imputing missing values

#### 1. determine the number of missing values in the *activity* dataset

```{r, echo=TRUE}
qMissingActivity <- sum(is.na(activity$steps), na.rm=FALSE) 
```
The count of the total missing values is **`r qMissingActivity`**

#### 2. fill in the missing data from the original dataset using 'MICE'
...
For the new dataset, the mean of the total steps per day is **`r mStepsPerDay[2,1]`** and the median is **`r mStepsPerDay[2,2]`**
...
The original dataset yielded a mean and median of **`r mStepsPerDay[1,1]`** and **`r mStepsPerDay[1,2]`**, respectively. After filling in the missing
data, designated by NAs, the mean and median are **`r mStepsPerDay[2,1]`**, and **`r mStepsPerDay[2,2]`**. The new values, while larger that the 
original numbers, seem reasonable, as the holes, which were labeled NA|0, have been filled with new values from the 
use of the MICE algorithm.
...
Inspecting the results of the panel-plot, there is a definate difference between weekday and weekend.
...
```{r}
        ...
        ggplot(newStepsPerInterval, aes(x=time, y=avgSteps)) +
            ...
            labs(x="Time Period Averaged, TOD", y="Number of Steps", title="Daily Activity, Averaged") +
...
### Addendum
##### Although this plot is not required, I felt in interesting and needed to be added to the report. 
Imagine 288 saws with uneven teeth stacked on top of each other. Each tooth represents an interval count, so moving down the saw-stack in the same tooth position, we have differing heights. These heights represent the total (day) counts for each interval, of which there are over 17,000 values in the 'activity' file. Each value is represented on the histogram as a line through a bar. The red line traces the degree of 'attraction' or 'repulsion' of values across each interval, pushing up and pulling it down across the plot. A better way to represent this data would be with a 3-D plot, but then, I haven't gone there ... yet.
